...
const [workoutExercises, setWorkoutExercises] = useState([
  ...
  function normalizeCompactText(value, maxLength = 22) {
  ...
  function getScheduleTodayBodyPart(schedule, workout) {
    ...
    [
      schedule?.today_body_part,
      schedule?.todayBodyPart,
      schedule?.body_part,
      schedule?.bodyPart,
      schedule?.workout_body_part,
      schedule?.workoutBodyPart,
      schedule?.planned_body_part,
      schedule?.plannedBodyPart,
    ].forEach((value) => {
      if (Array.isArray(value)) candidates.push(value.join(", "));
  ...
  function getScheduleConditionCompact(schedule, member, workout) {
    ...
    const raw = workout?.condition || latestCondition?.condition || latestCondition?.status || "";
    const text = getWorkoutConditionText(raw) || getConditionPreviewText(latestCondition)?.split(" · ")[0] || "보통";

    if (String(text).includes("좋")) return { text: "좋음", icon: "🙂" };
    if (String(text).includes("나쁨") || String(text).includes("안좋") || String(text).includes("불편")) return { text: "나쁨", icon: "😟" };
    return { text: "보통", icon: "😐" };
  }
        ...
        {schedules.length === 0 ? (
          ...
          <div style={styles.incompleteList}>
                ...
                <div key={schedule.id} style={styles.incompleteItem}>
                  <div style={styles.compactScheduleHead}>
                    <div style={styles.compactScheduleTime}>
                      ...
                      <strong>{formatTime(schedule.start_time).replace(/^오후\s?/, "").replace(/^오전\s?/, "")}</strong>
                    </div>
                      ...
                      {preferenceTags.length > 0 && (
                        <div style={styles.compactTagRow}>
                          {preferenceTags.slice(0, 3).map((tag) => (
                            <span key={tag} style={styles.compactTag}>
                              {tag}
                            </span>
                          ))}
                        </div>
                  ...
                  <div style={styles.scheduleStatusRow}>
                    ...
                    {isCancelled ? (
                      ...
                      <>
                        <span style={attended ? styles.scheduleDoneText : styles.scheduleWarningText}>
                          {attended ? "출석 완료" : "출석 전"}
                        </span>
                        <span style={ptUsed ? styles.scheduleDoneText : styles.scheduleWarningText}>
                          {ptUsed ? "차감 완료" : "차감 전"}
                        </span>
                      </>
                    )}
            ...
            >
              ...
              <strong style={styles.moreScheduleCount}>더보기</strong>
...
const styles = {
  ...
  page: {
    ...
    minHeight: "100dvh",
    overflow: "hidden",
    background: "radial-gradient(circle at 50% -10%, rgba(246,211,139,0.08), transparent 34%), linear-gradient(180deg, #05080a 0%, #090d0f 100%)",
    color: "#fff",
    padding: "18px 26px max(12px, env(safe-area-inset-bottom))",
    boxSizing: "border-box",
  ...
  notice: {
    ...
    left: 24,
    right: 24,
    bottom: "calc(max(62px, env(safe-area-inset-bottom)) + 126px)",
    zIndex: 80,
    background: "linear-gradient(90deg, rgba(82,59,14,0.74), rgba(32,25,14,0.88)) padding-box, linear-gradient(135deg, rgba(246,211,139,0.72), rgba(214,157,48,0.42)) border-box",
...
textarea: {
  ...
  page: {
    ...
    display: "flex",
    flexDirection: "column",
    background: "radial-gradient(circle at 10% 0%, rgba(32,38,39,0.72) 0%, rgba(8,12,14,0.96) 42%, #030506 100%)",
    color: "#fff",
    padding: "10px 22px max(62px, env(safe-area-inset-bottom))",
    fontFamily: "Arial, sans-serif",
  },
  header: {
    display: "flex",
  ...
  incompleteBox: {
    background: "linear-gradient(180deg, rgba(19,24,26,0.92), rgba(5,9,11,0.98)) padding-box, linear-gradient(135deg, rgba(255,230,166,0.92), rgba(190,126,22,0.52), rgba(255,238,190,0.72)) border-box",
    border: "1.15px solid transparent",
    borderRadius: 18,
    padding: "17px 18px 16px",
    marginBottom: 20,
    height: "clamp(430px, calc(100svh - 310px), 520px)",
    minHeight: 0,
    boxSizing: "border-box",
    boxShadow: "inset 0 1px 0 rgba(255,255,255,0.05), 0 14px 36px rgba(0,0,0,0.28), 0 0 18px rgba(214,157,48,0.06)",
    flexShrink: 0,
  },
  incompleteTop: {
    display: "flex",
  ...
  incompleteList: {
    display: "grid",
    gridTemplateColumns: "repeat(5, minmax(0, 1fr))",
    gridTemplateRows: "minmax(0, 1fr) 66px",
    gap: 12,
    alignItems: "stretch",
  ...
  incompleteItem: {
    position: "relative",
    background: "linear-gradient(180deg, rgba(22,27,29,0.88), rgba(6,10,12,0.98)) padding-box, linear-gradient(145deg, rgba(255,224,148,0.80), rgba(176,111,16,0.46), rgba(246,211,139,0.62)) border-box",
    border: "1px solid transparent",
    borderRadius: 11,
    ...
    minWidth: 0,
    minHeight: 0,
    boxShadow: "inset 0 1px 0 rgba(255,255,255,0.045), 0 8px 18px rgba(0,0,0,0.20)",
  },
  compactScheduleHead: {
    display: "grid",
  ...
  compactWorkoutRow: {
    display: "grid",
    gridTemplateColumns: "40px minmax(42px, 1fr) 1px 28px auto",
    alignItems: "center",
    gap: 8,
    minHeight: 52,
  },
  compactBodyIcon: {
    ...
    alignItems: "center",
    justifyContent: "center",
    border: "1px solid rgba(246,211,139,0.82)",
    color: "#f6d38b",
    fontSize: 13,
    fontWeight: 1000,
    background: "rgba(246,211,139,0.035)",
  },
  compactBodyText: {
    fontSize: 22,
    fontWeight: 1000,
  ...
  scheduleDoneText: {
    color: "#d7fff3",
    background: "rgba(32,92,78,0.42)",
    border: "1px solid rgba(62,151,130,0.5)",
    borderRadius: 999,
    padding: "5px 9px",
    fontSize: 11,
    fontWeight: 1000,
  ...
  scheduleWarningText: {
    color: "#f6d38b",
    background: "rgba(70,48,12,0.45)",
    border: "1px solid rgba(214,157,48,0.50)",
    borderRadius: 999,
    padding: "5px 9px",
    fontSize: 11,
    fontWeight: 1000,
  ...
  homeLauncherItem: {
    ...
    minHeight: 104,
    border: "1px solid transparent",
    background: "linear-gradient(180deg, rgba(23,28,30,0.88), rgba(9,13,15,0.97)) padding-box, linear-gradient(145deg, rgba(255,224,148,0.72), rgba(164,103,14,0.38), rgba(246,211,139,0.56)) border-box",
    color: "#fff",
    borderRadius: 12,
    padding: "9px 7px",
    display: "grid",
    ...
    textAlign: "center",
    fontSize: 10,
    boxShadow: "inset 0 1px 0 rgba(255,255,255,0.04), 0 8px 16px rgba(0,0,0,0.15)",
  },
  homeLauncherIcon: {
    height: 28,
    minWidth: 28,
  ...
  todayScheduleOnlyModal: {
    ...
    border: "1.8px solid rgba(246,211,139,0.82)",
    borderRadius: 24,
    padding: "18px 22px 22px",
    color: "#fff",
    boxShadow: "0 24px 80px rgba(0,0,0,0.55), inset 0 1px 0 rgba(255,255,255,0.06), 0 0 24px rgba(246,211,139,0.12)",
    boxSizing: "border-box",
  },
  ...
  todayScheduleOnlyCard: {
    ...
    border: "1.35px solid rgba(246, 211, 139, 0.68)",
    borderRadius: 15,
    ...
    gap: 5,
    minWidth: 0,
    boxShadow: "inset 0 1px 0 rgba(255,255,255,0.05), 0 8px 18px rgba(0,0,0,0.18), 0 0 10px rgba(246,211,139,0.08)",
  },
  ...
  moreScheduleCard: {
    ...
    gridColumn: "1 / -1",
    minHeight: 58,
    background: "linear-gradient(180deg, rgba(18,23,25,0.62), rgba(7,11,13,0.92)) padding-box, linear-gradient(135deg, rgba(255,255,255,0.16), rgba(246,211,139,0.28), rgba(255,255,255,0.10)) border-box",
    border: "1px solid transparent",
    borderRadius: 11,
    padding: "6px",
    minWidth: 0,
    ...
    gap: 10,
    textAlign: "center",
    boxShadow: "inset 0 1px 0 rgba(255,255,255,0.045), 0 8px 18px rgba(0,0,0,0.12)",
  },
  moreScheduleIcon: {
    display: "none",
  },
  ...
  modalBox: {
    ...
    background: "linear-gradient(180deg, #fffdf8 0%, #f8f5ed 100%)",
    color: "#111827",
    border: "1px solid rgba(191, 159, 104, .42)",
    borderRadius: 28,
    padding: 14,
    boxShadow: "0 22px 64px rgba(15, 23, 42, .26)",
  },
  detailTop: {
    display: "flex",
    justifyContent: "space-between",
    ...
    border: "1px solid rgba(191, 159, 104, .25)",
  },
  detailNameLine: {
    display: "flex",
    alignItems: "center",
  ...
  detailSummaryPanel: {
    ...
    border: "1px solid rgba(191, 159, 104, .30)",
    borderRadius: 20,
    padding: 12,
    boxShadow: "0 10px 24px rgba(17,24,39,.045)",
  },
  detailPanelTopRow: {
    display: "flex",
    justifyContent: "space-between",
  ...
  menuButton: {
    ...
    alignItems: "center",
    minHeight: 52,
    boxShadow: "0 8px 18px rgba(17,24,39,.045)",
    textAlign: "left",
  },
  menuButtonIcon: {
    width: 22,
